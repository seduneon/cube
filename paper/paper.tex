...
the large \textsc{MLP} (${\sim}$169K parameters) achieves Pearson $r = 0.81$
yet a 0\% beam-search solve rate at every depth—demonstrating that even
...
In a controlled comparison at equal parameter count (${\sim}$40K), both
equivariant models substantially outperform the unconstrained \textsc{MLP}
...
the value landscape, not just average prediction accuracy: a supplementary
large \textsc{MLP} with $4\times$ more parameters achieves Pearson
$r = 0.81$ yet 0\% solve rate, while equivariant models succeed reliably.
