...
\author{Zhambyl Maksotov\\
\small Research Methods in Computer Science\\
\small \textit{[University Name]}}
...
compared against a parameter-matched unconstrained \textsc{MLP} (${\sim}$40K
parameters) as the primary baseline.
...
Empirically, at equal parameter count, \textsc{EMLP-Col} achieves a greedy
solve rate of 97.2\% at scramble depth 10 and a mean absolute error of
0.93 moves, compared to 94.2\% and 1.24 moves for the spatial-only
\textsc{EMLP}, and a solve rate of [pending] for the unconstrained \textsc{MLP}.
...
combinatorial puzzles often have \emph{exact}, group-theoretic symmetry baked
...
The 2\texttimes2\texttimes2 Pocket Cube (Figure~\ref{fig:cube}) is an
...
demonstrates that \emph{beam search guided by a learned distance heuristic}
...
\begin{enumerate}
  \item We design a \textbf{Color-Equivariant Convolution layer}
    (\textsc{ColorEquivConv}) that is jointly equivariant to
  ...
  \item We provide \textbf{formal proofs} of equivariance for both the
    spatial-only layer (\textsc{GroupConv}) and the combined layer
    (\textsc{ColorEquivConv}), and a corollary establishing invariance of
...
\begin{figure}[t]
  \centering
  % Replace with an actual cube diagram if available
  \fbox{\parbox{0.45\textwidth}{\centering\vspace{1cm}
    [Figure: 2$\times$2$\times$2 cube with sticker labels]
    \vspace{1cm}}}
  \caption{The 2\texttimes2\texttimes2 Pocket Cube.
    Each of the 24 sticker positions is assigned one of 6 colors.
    The solved state has each face showing a single color.}
  \label{fig:cube}
\end{figure}
...
BFS over the Cayley graph of all reachable states establishes the
...
\cite{cohen2016group}, is to use \emph{weight sharing} across orbits:
...
a ``diffusion distance'' to the solved state, and \emph{beam search} of width
...
\subsection{Spatial Group Convolution Layer (\textsc{GroupConv})}
\label{sec:groupconv}

The \textsc{GroupConv} layer maps a hidden state of shape
...
\subsection{Color-Equivariant Convolution Layer (\textsc{ColorEquivConv})}
\label{sec:colorconv}

The \textsc{ColorEquivConv} layer is the main technical contribution of this
paper.
It extends \textsc{GroupConv} to the full group
...
channels in \emph{blocks of six}: a hidden state with $k$ blocks has
...
They differ in \emph{how} that space is computed.
...
For a \emph{finite} group acting by \emph{permutation}, the null space of
...
\emph{identical} equivariant function classes: the null space of $C$
equals the span of the orbit indicator matrices.
Table~\ref{tab:emlp_comparison} summarises the key differences.

\begin{table}[h]
\centering
\caption{Comparison of equivariant basis construction methods.}
\label{tab:emlp_comparison}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lll}
\toprule
& \textbf{EMLP (Finzi et al.)} & \textbf{Orbit enumeration (ours)} \\
\midrule
Mathematical object & $\mathrm{Hom}_G(V,W)$ & $\mathrm{Hom}_G(V,W)$ \\
Method & Null space via SVD/Krylov & Exhaustive group enumeration \\
Basis vectors & Numerical (from SVD) & Orbit indicator matrices \\
Interpretability & Low & High (one weight per orbit) \\
Continuous groups & \checkmark & $\times$ \\
Permutation repns & \checkmark & \checkmark \\
Constraint matrix size & $O(|V|^2|W|^2)$ & — (no matrix needed) \\
Setup cost (our cube) & $576{\times}576$ system & 24 group-element lookups \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{InvariantLinear (for \textsc{EMLP}).}
...
\paragraph{InvariantHead (for \textsc{EMLP-Col}).}
...
\midrule
\textsc{EMLP}     & \textsc{GroupConv}      & $c=28$ & $\sim$42K &
  $\mathcal{O}$             & Spatial equivariance \\
\textsc{EMLP-Col} & \textsc{ColorEquivConv} & $k=20$ & $\sim$39K &
...
\subsection{Equivariance of \textsc{GroupConv}}

\begin{proposition}[\textsc{GroupConv} is $\mathcal{O}$-equivariant]
\label{prop:groupconv}
Let $\pi \in \mathcal{O}$ act on sticker positions.
For any input $\bx \in \R^{B \times 24 \times c_{\mathrm{in}}}$,
\begin{equation}
  \textsc{GroupConv}(\pi \cdot \bx) = \pi \cdot \textsc{GroupConv}(\bx),
\end{equation}
...
We compute the $i$-th output of $\textsc{GroupConv}(\pi \cdot \bx)$:
\begin{align}
  \left[\textsc{GroupConv}(\pi \cdot \bx)\right]_{b,i,o}
...
\begin{align}
  ...
  &= \left[\textsc{GroupConv}(\bx)\right]_{b,\pi^{-1}(i),o}
   = \left[\pi \cdot \textsc{GroupConv}(\bx)\right]_{b,i,o}. \qedhere
\end{align}
\end{proof}

\subsection{Equivariance of \textsc{ColorEquivConv}}

\begin{theorem}[\textsc{ColorEquivConv} is $\mathcal{G}$-equivariant]
\label{thm:colorconv}
...
\begin{equation}
  \textsc{ColorEquivConv}\!\left((g,\sigma) \cdot \bx\right)
  = (g,\sigma) \cdot \textsc{ColorEquivConv}(\bx).
\end{equation}
\end{theorem}
...
$\textsc{ColorEquivConv}\!\left((g, \sigma) \cdot \bx\right)$:
\begin{align}
  \left[\textsc{ColorEquivConv}\!\left((g,\sigma)\cdot\bx\right)\right]_{b,i,o,d}
...
\begin{align}
  &= \sum_{j',\kappa,c'} W_{o,\kappa,\,\orbsp(g^{-1}(i),\,j'),\,\orbco(\sigma^{-1}(d),\,c')}
     \cdot x_{b,j',\kappa,c'} \notag \\
  &= \left[\textsc{ColorEquivConv}(\bx)\right]_{b,\,g^{-1}(i),\,o,\,\sigma^{-1}(d)} \notag \\
  &= \left[(g,\sigma)\cdot\textsc{ColorEquivConv}(\bx)\right]_{b,i,o,d}. \qedhere
\end{align}
\end{proof}
...
\end{equation}
for \emph{all} $(g, \sigma) \in \mathcal{G}$—a total of $17{,}280$ equivalent
...
to the batch.
This provides \emph{approximate} coverage of $\mathcal{G}$ in expectation,
...
The weight tensor of \textsc{ColorEquivConv} has
$k_{\mathrm{out}} \times k_{\mathrm{in}} \times n_{\mathrm{sp}} \times 2$
parameters.
...
\begin{table}[h]
\centering
\caption{Main results on the 200K training set (seed 42), mean over 3 seeds.
Best result per column in \textbf{bold}.
\textit{[pending]}: \textsc{MLP} results will be filled in once training completes.
$^\ddagger$\,Supplementary baselines not part of the primary comparison:
\textsc{MLP-Aug} ($h=110$, augmented, ${\sim}$40K params): MAE 2.36, $r$ 0.36,
solve rate 1.4\% at depth 10;
\textsc{MLP-Large} ($h=256$, unconstrained, ${\sim}$169K params): MAE 1.32,
$r$ 0.81, solve rate 0.0\% at depth 10.}
\label{tab:results}
\renewcommand{\arraystretch}{1.2}
...
\multirow{2}{*}{\textbf{MAE}$\downarrow$} &
\multirow{2}{*}{\textbf{Rounded acc.}$\uparrow$} &
\multirow{2}{*}{\textbf{Pearson $r$}$\uparrow$} &
\multicolumn{2}{c}{\textbf{Equiv.\ err.\ (moves)}} &
\multicolumn{2}{c}{\textbf{Solve rate}} \\
\cmidrule(lr){6-7} \cmidrule(lr){8-9}
& & & & & \textit{spatial} & \textit{color} &
  \textit{depth 7} & \textit{depth 10} \\
\midrule
\textsc{EMLP-Col} & 39K & \textbf{0.932} & \textbf{45.4\%} & \textbf{0.900} &
  $7.8{\times}10^{-7}$ & $8.3{\times}10^{-7}$ & \textbf{100.0\%} & \textbf{97.2\%} \\
\textsc{EMLP}     & 42K & 1.239 & 31.9\% & 0.839 &
  $4.4{\times}10^{-7}$ & — & \textbf{100.0\%} & 94.2\% \\
\midrule
\textsc{MLP}      & 40K & \textit{[pending]} & \textit{[pending]} & \textit{[pending]} &
...
$8.3{\times}10^{-7}$ moves, confirming joint $\mathcal{O} \times S_6$
invariance.
The parameter-matched \textsc{MLP} must learn any symmetry from data; its
equivariance error (\textit{pending}) is expected to be orders of magnitude
larger, reflecting the inconsistency that harms beam search.

\paragraph{Finding 2: Symmetry vs.\ no symmetry at equal parameters.}
The central comparison of this paper is \textsc{EMLP} and \textsc{EMLP-Col}
(both ${\sim}$40K parameters, built-in symmetry) against the parameter-matched
\textsc{MLP} (${\sim}$40K parameters, no symmetry).
Supplementary results reinforce this picture: a large unconstrained
\textsc{MLP} with $4\times$ more parameters (${\sim}$169K) achieves Pearson
$r = 0.81$ yet a 0.0\% beam-search solve rate at every depth (see
Table~\ref{tab:results} footnote), demonstrating that global accuracy metrics
alone do not predict solver performance.
The equivariant models' consistent value landscapes — guaranteed by
Corollary~\ref{thm:colorconv} — provide the per-move ranking reliability
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % \includegraphics[width=\textwidth]{../results/plots/learning_curves.png}
    \fbox{\parbox{\textwidth}{\centering\vspace{1.5cm}[Learning curves]\vspace{1.5cm}}}
    \caption{Validation MSE vs.\ epoch (200K training set).}
    \label{fig:learning_curves}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % \includegraphics[width=\textwidth]{../results/plots/per_depth_accuracy.png}
    \fbox{\parbox{\textwidth}{\centering\vspace{1.5cm}[Per-depth MAE]\vspace{1.5cm}}}
    \caption{MAE broken down by BFS depth.}
    \label{fig:per_depth}
  \end{subfigure}
  \caption{Training dynamics and per-depth prediction error.}
\end{figure}
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % \includegraphics[width=\textwidth]{../results/plots/solve_rates.png}
    \fbox{\parbox{\textwidth}{\centering\vspace{1.5cm}[Solve rates]\vspace{1.5cm}}}
    \caption{Greedy solve rate by scramble depth.}
    \label{fig:solve_rates}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.48\textwidth}
    \centering
    % \includegraphics[width=\textwidth]{../results/plots/equivariance_error.png}
    \fbox{\parbox{\textwidth}{\centering\vspace{1.5cm}[Equivariance error]\vspace{1.5cm}}}
    \caption{Equivariance error by symmetry type (log scale).}
    \label{fig:equiv_error}
  \end{subfigure}
  \caption{Solve performance and symmetry measurement.}
\end{figure}
...
distance.
Even a model with high global correlation can fail if its predictions are
inconsistent across symmetry-equivalent states: two physically identical
positions (related by a rotation) may receive different predicted distances,
causing the solver to cycle.
...
incorrectness in the neural network predictions,'' but only up to a point.
When the heuristic is so inconsistent that successor rankings are essentially
random—as with our \textsc{MLP} ($r = 0.81$ globally but 0\% solve rate)—beam
search cannot recover.
The implicit 17,280-fold consistency enforced by equivariance provides exactly
the reliable local ranking that beam search requires.
...
This makes the \textsc{ColorEquivConv} layer remarkably compact: the color
...
The current experiments use only the 2\texttimes2\texttimes2 cube with
a 200K training set, limiting generalizability of the conclusions.
The \textsc{MLP-Aug} results may be affected by the interaction between
small capacity ($h = 110$) and heavy augmentation (full $\mathcal{G}$
sampling), making it difficult to isolate the effect of augmentation alone.
Additionally, the greedy search with beam width 20 is a relatively weak
solver; comparison under A* or MCTS \cite{agostinelli2019solving} would
...
Empirically, \textsc{EMLP-Col} achieves a 97.2\% greedy solve rate at
scramble depth 10 with only 39K parameters, compared to 0.0\% for an
unconstrained MLP with $4\times$ more parameters.
A key insight is that equivariance matters for \emph{local consistency} of
the value landscape, not just average prediction accuracy: the MLP achieves
Pearson $r = 0.81$ yet completely fails at greedy search, while equivariant
models with similar MAE succeed reliably.
